...
public class ActivateOnController : MonoBehaviour
{
	...
	private void Check()
	{
		if (!oneTime || !doneOnce)
		{
			if ((!doneOnce || activated) && (Gamepad.current == null || !((InputDevice)Gamepad.current).enabled))
			{
				activated = false;
				onController?.Revert();
				onNoController?.Invoke();
			}
			else if (!activated && Gamepad.current != null && ((InputDevice)Gamepad.current).enabled)
			{
				activated = true;
				onController?.Invoke();
