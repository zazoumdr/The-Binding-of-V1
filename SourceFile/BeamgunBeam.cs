...
public class BeamgunBeam : MonoBehaviour
{
	...
	private void Start()
	{
		...
		((Component)(object)hitParticle).transform.localScale = Vector3.one * beamWidth * 5f;
	}
	...
	private void Update()
	{
		...
		if (active)
		{
			...
			if (line.widthMultiplier != beamWidth)
			{
				...
				((Component)(object)hitParticle).transform.localScale = Vector3.one * line.widthMultiplier * 5f;
			}
			...
			if (Physics.Raycast(base.transform.position, base.transform.forward, out var hitInfo, float.PositiveInfinity, layerMask, QueryTriggerInteraction.Ignore))
			{
				...
				else if (hitInfo.transform.gameObject.CompareTag("Breakable") && hitInfo.transform.TryGetComponent<Breakable>(out component2) && !component2.precisionOnly && !component2.specialCaseOnly)
				{
					component2.Break(0.15f);
				}
				hitPosition = hitInfo.point;
				forward = hitInfo.normal;
			}
			line.SetPosition(1, hitPosition);
			((Component)(object)hitParticle).transform.position = hitPosition;
			((Component)(object)hitParticle).transform.forward = forward;
