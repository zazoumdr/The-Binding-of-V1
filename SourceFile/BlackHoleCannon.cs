...
public class BlackHoleCannon : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (!MonoSingleton<InputManager>.Instance.PerformingCheatMenuCombo() && MonoSingleton<InputManager>.Instance.InputSource.Fire1.WasPerformedThisFrame)
		{
			...
			{
				aud.PlayOneShot(emptyClick, 1f, tracked: true);
			}
		}
	...
	private void Shoot()
	{
		...
		if (Physics.Raycast(cam.transform.position, cam.transform.forward, out rhit, float.PositiveInfinity, LayerMaskDefaults.Get(LMD.Environment)))
		{
