...
public class CustomPatterns : MonoBehaviour
{
	...
	public void Toggle()
	{
		...
		stateButtonText.text = (customPatternMode ? "Enable" : "Disable");
		enableWhenCustom?.SetActive(!customPatternMode);
		disableWhenCustom?.SetActive(customPatternMode);
	...
	public void SaveEnabledPatterns()
	{
		...
		MonoSingleton<PrefsManager>.Instance.SetStringLocal("cyberGrind.enabledPatterns", JsonUtility.ToJson((object)activePatterns));
	}
	...
	public void LoadEnabledPatterns()
	{
		...
		stateButtonText.text = (flag ? "Enable" : "Disable");
		enableWhenCustom.SetActive(!flag);
		disableWhenCustom?.SetActive(flag);
	}
	...
	public void BuildButtons()
	{
		...
		for (int num = 2; num < grid.childCount; num++)
		{
			Object.Destroy(grid.GetChild(num).gameObject);
		}
		maxPages = Mathf.CeilToInt((float)list.Count / (float)maxItemsPerPage);
		for (int num2 = (currentPage - 1) * maxItemsPerPage; num2 < list.Count && num2 < currentPage * maxItemsPerPage; num2++)
		{
			GridTile tile = list[num2];
			if (tile.folder)
			{
				...
				for (int num3 = 0; num3 < 48; num3++)
				{
					...
					{
						target.SetPixel(num3, num4, Color.black);
					}
				}
				for (int num5 = 0; num5 < array.Length && num5 < 7; num5++)
				{
					ArenaPattern pattern = LoadPattern(Path.Combine(tile.path, array[num5]));
					Vector2Int offset = new Vector2Int(num5 % 3, (num5 > 2) ? 1 : 0);
			...
			bool num6 = GeneratePatternPreview(arenaPattern, Vector2Int.zero, ref target2);
			target2.Apply();
			if (!num6)
			{
