...
public class CyberGrindNavHelper : MonoBehaviour
{
	...
	private void CheckIfBridge(EndlessCube dom, EndlessCube mid, EndlessCube sub, BridgeDirection dir)
	{
		if (sub.transform.position.y == dom.transform.position.y && !(dom.transform.position.y - mid.transform.position.y <= 10f) && (dir != BridgeDirection.Bottom || !bridges[dom.positionOnGrid.x][dom.positionOnGrid.y].BottomBridge) && (dir != BridgeDirection.Top || !bridges[dom.positionOnGrid.x][dom.positionOnGrid.y].TopBridge) && (dir != BridgeDirection.Left || !bridges[dom.positionOnGrid.x][dom.positionOnGrid.y].LeftBridge) && (dir != BridgeDirection.Right || !bridges[dom.positionOnGrid.x][dom.positionOnGrid.y].RightBridge))
		{
			...
			NavMeshLink obj2 = obj.AddComponent<NavMeshLink>();
			...
			normalized *= 1.75f;
			obj2.startPoint = normalized;
			obj2.endPoint = sub.transform.position - dom.transform.position - normalized;
			obj2.bidirectional = true;
			obj2.width = 3f;
			obj2.UpdateLink();
			switch (dir)
			{
	...
	private void CheckNeighbors(Transform dom, Transform sub)
	{
		if (!(sub.position.y >= dom.position.y) && !(dom.position.y - sub.position.y > 10f))
		{
			...
			normalized *= 1.75f;
			obj2.startPoint = normalized;
			obj2.endPoint = sub.position - dom.position;
			obj2.bidirectional = true;
			obj2.width = 5f;
			obj2.UpdateLink();
		}
