...
public class Door : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		if ((Object)(object)nmo != null && (startOpen || open))
		{
			((Behaviour)(object)nmo).enabled = false;
		}
		...
		else if ((docons == null || docons.Length == 0) && !LayerMaskDefaults.IsMatchingLayer(base.gameObject.layer, LMD.Environment))
		{
		...
		{
			if ((bool)meshRenderer && (bool)meshRenderer.sharedMaterial && meshRenderer.sharedMaterial.IsKeywordEnabled("EMISSIVE"))
			{
	...
	private void Update()
	{
		if (doorType != DoorType.Normal || inPos)
		{
			return;
		}
		float num = Vector3.Distance(base.transform.localPosition, targetPos) / Mathf.Min(Vector3.Distance(closedPos, openPosRelative), 100f);
		float num2 = Mathf.Min(num * 10f, 1f);
		if (!useRigidbody || !rb)
		{
			base.transform.localPosition = Vector3.MoveTowards(base.transform.localPosition, targetPos, Time.deltaTime * (easeIn ? Mathf.Min(speed, num * 2f * speed + speed / 50f) : speed));
		}
		if (screenShake)
		{
			if (cc == null)
			...
			cc.CameraShake(0.05f * num2);
		}
		if ((bool)(Object)(object)aud && easeIn)
		{
			aud.volume = num2 * origVol;
			aud.SetPitch(num2 * origPitch);
		}
		if (!(Vector3.Distance(base.transform.localPosition, targetPos) < Mathf.Min(0.1f, Vector3.Distance(closedPos, openPosRelative) / 1000f)))
		{
			return;
		}
		...
		if ((bool)(Object)(object)aud)
		{
			if ((Object)(object)closeSound != null)
			{
				aud.clip = closeSound;
				aud.loop = false;
				aud.Play(tracked: true);
				aud.volume = origVol;
				aud.SetPitch(origPitch);
			}
			else
			{
				aud.Stop();
		...
		if ((Object)(object)nmo != null)
		{
			if (base.transform.localPosition == closedPos)
			{
				((Behaviour)(object)nmo).enabled = true;
			}
			else
			{
				((Behaviour)(object)nmo).enabled = false;
	...
	{
		if (doorType == DoorType.Normal && !inPos && useRigidbody && (bool)rb)
		{
			Vector3 target = (base.transform.parent ? base.transform.parent.TransformPoint(targetPos) : targetPos);
	...
	public void Open(bool enemy = false, bool skull = false)
	{
		...
		{
			GameObject[] array2 = activatedRooms;
		...
		open = true;
		isFullyClosed = false;
		if (doorHandles != null && doorHandles.Length != 0)
		{
			Animator[] array3 = doorHandles;
			for (int i = 0; i < array3.Length; i++)
			{
				array3[i].SetBool("Open", true);
			}
		}
		if ((bool)occpor)
		{
		...
		if (!enemy && !dontCloseOtherDoorsWhenOpening && docons.Length != 0)
		{
			...
			Door[] array4 = allDoors;
			foreach (Door door in array4)
			{
		...
		else
		{
			...
			{
				BigDoor[] array5 = bdoors;
				foreach (BigDoor bigDoor in array5)
				{
					if (reverseDirection)
					{
			...
			{
				SubDoor[] array6 = subdoors;
				foreach (SubDoor subDoor in array6)
				{
					if (!subDoor.dr)
					{
						subDoor.dr = this;
	...
	public void Close(bool force = false)
	{
		...
		else
		{
			...
			if (doorHandles != null && doorHandles.Length != 0)
			{
				Animator[] array = doorHandles;
				for (int i = 0; i < array.Length; i++)
				{
					array[i].SetBool("Open", false);
				}
			}
			if (requests > 0 && !force)
			{
			...
			{
				SubDoor[] array3 = subdoors;
				foreach (SubDoor subDoor in array3)
				{
					if (!subDoor.dr)
					{
						subDoor.dr = this;
	...
	public void Lock()
	{
		...
		else
		{
			...
			if ((bool)(Object)(object)aud2)
			{
				aud2.SetPitch(0.2f);
				aud2.Play(tracked: true);
			}
	...
	public void Unlock()
	{
		...
		if (locked && (bool)(Object)(object)aud2)
		{
			aud2.SetPitch(0.5f);
			aud2.Play(tracked: true);
		}
