...
public class FishBait : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (returnToRod)
		{
			...
			if (fishPullVelocity > 0.1f)
			{
				...
				{
					sourceWeapon.pullSound.Play(tracked: true);
				}
				sourceWeapon.pullSound.SetPitch(Mathf.Abs(0.7f + fishPullVelocity * 2f));
			}
			else
			{
	...
	private void ThrowAnim()
	{
		...
		float num = Mathf.Sin(flyProgress * MathF.PI) * 20f;
