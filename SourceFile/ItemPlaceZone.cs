...
public class ItemPlaceZone : MonoBehaviour
{
	...
	public void CheckItem(bool prelim = false)
	{
		...
		GameObject[] array;
		Door[] array2;
		InstantiateObject[] array3;
		if (componentInChildren != null)
		{
			if (componentInChildren.itemType == acceptedItemType)
			{
				if (!prelim && !hideEffects && !acceptedItemPlaced)
				{
					if ((bool)(Object)(object)elementChangeEffect)
					{
						elementChangeEffect.Play();
					}
					if ((bool)(Object)(object)soundOnActivated)
					{
						Object.Instantiate<AudioSource>(soundOnActivated, base.transform.position, Quaternion.identity);
					}
				}
				...
				if (!hideEffects)
				{
					array3 = altarElements;
					foreach (InstantiateObject instantiateObject in array3)
					{
						instantiateObject.gameObject.SetActive(value: true);
				...
				if (!prelim)
				{
					ArenaStatus[] array4 = arenaStatuses;
					for (int i = 0; i < array4.Length; i++)
					{
						array4[i].currentStatus++;
					}
					array4 = reverseArenaStatuses;
					for (int i = 0; i < array4.Length; i++)
					{
						array4[i].currentStatus--;
					}
		...
		if (!prelim)
		{
			...
			if (!hideEffects)
			{
				...
				if ((bool)(Object)(object)soundOnDeactivated)
				{
					Object.Instantiate<AudioSource>(soundOnDeactivated, base.transform.position, Quaternion.identity);
				}
			}
			ArenaStatus[] array4 = arenaStatuses;
			for (int i = 0; i < array4.Length; i++)
