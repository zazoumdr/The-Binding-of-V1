...
public class LightningStrikeExplosive : MonoBehaviour
{
	...
	private void Start()
	{
		...
		bool flag = false;
		if ((bool)MonoSingleton<CoinTracker>.Instance && MonoSingleton<CoinTracker>.Instance.revolverCoinsList.Count > 0)
		{
			for (int i = 0; i < MonoSingleton<CoinTracker>.Instance.revolverCoinsList.Count; i++)
			{
				if (MonoSingleton<CoinTracker>.Instance.revolverCoinsList[i].transform.position.y > MonoSingleton<PlayerTracker>.Instance.GetPlayer().position.y && Vector2.Distance(new Vector2(MonoSingleton<CoinTracker>.Instance.revolverCoinsList[i].transform.position.x, MonoSingleton<CoinTracker>.Instance.revolverCoinsList[i].transform.position.z), new Vector2(base.transform.position.x, base.transform.position.z)) < 2f)
				{
					flag = true;
