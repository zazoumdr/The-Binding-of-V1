...
public class MapFloatSetter : MapVarSetter
{
	...
	public override void SetVar()
	{
		...
		case FloatInputType.AddNumber:
			MonoSingleton<MapVarManager>.Instance.SetFloat(variableName, MonoSingleton<MapVarManager>.Instance.GetFloat(variableName).GetValueOrDefault() + number);
			break;
		case FloatInputType.CopyDifferentVariable:
			MonoSingleton<MapVarManager>.Instance.SetFloat(variableName, MonoSingleton<MapVarManager>.Instance.GetFloat(sourceVariableName).GetValueOrDefault());
			break;
