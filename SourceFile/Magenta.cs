...
public class Magenta
{
	...
	public Magenta(string path, bool loadUnhardened = true)
	{
		...
		bundles = JsonConvert.DeserializeObject<Dictionary<string, List<string>>>(text);
		...
		string text2 = Path.Combine(path, "sceneDependencies.json");
		if (File.Exists(text2))
		{
			string text3 = File.ReadAllText(text2);
			sceneDependencies = JsonConvert.DeserializeObject<Dictionary<string, List<string>>>(text3);
		}
		string text4 = Path.Combine(path, "agonyDependencies.json");
		if (File.Exists(text4))
		{
			string text5 = File.ReadAllText(text4);
			agonyDependencies = JsonConvert.DeserializeObject<List<string>>(text5);
		}
	}
