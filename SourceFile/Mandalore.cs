...
public class Mandalore : MonoBehaviour
{
	...
	private void Update()
	{
		...
		else if (!voices[0].talking && !voices[1].talking && eid.target != null)
		{
			if (Random.Range(0f, 1f) > fullerChance)
			{
				...
				aud.PlayOneShot(voiceFull, tracked: true);
			...
			else
			{
				...
				aud.PlayOneShot(voiceFuller, tracked: true);
	...
	public void FullBurst()
	{
		if (!eid.dead)
		{
			...
			componentInChildren.safeEnemyType = EnemyType.Mandalore;
	...
	{
		Physics.IgnoreCollision(GetComponent<Collider>(), shammyCollider);
	}
}
