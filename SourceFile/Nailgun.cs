...
public class Nailgun : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		if (variation != 2)
		{
			...
			if ((bool)(Object)(object)heatSlider)
			{
				sliderBg = ((Component)(object)heatSlider).GetComponentInParent<Image>();
			}
		}
		if (!altVersion)
		{
			heatSteam = GetComponentInChildren<ParticleSystem>();
			heatSteamAud = ((Component)(object)heatSteam).GetComponent<AudioSource>();
		}
	...
	private void Start()
	{
		...
		if ((bool)(Object)(object)barrelAud)
		{
			AudioSource obj2 = barrelAud;
			obj2.volume -= wid.delay * 2f;
			if (barrelAud.volume < 0f)
			{
				barrelAud.volume = 0f;
			}
		}
		if ((bool)(Object)(object)heatSteamAud)
		{
			AudioSource obj3 = heatSteamAud;
			obj3.volume -= wid.delay * 2f;
	...
	private void OnEnable()
	{
		...
		if (variation == 1 && (bool)(Object)(object)aud)
		{
	...
	private void Update()
	{
		...
		if (burnOut || heatSinks < 1f)
		{
			...
			{
				burnOut = false;
				ParticleSystem obj = heatSteam;
				if (obj != null)
				{
					obj.Stop();
				}
				AudioSource obj2 = heatSteamAud;
				if (obj2 != null)
				{
					obj2.Stop();
				}
		...
		if ((bool)(Object)(object)heatSlider)
		{
			if (heatSlider.value != heatUp)
			{
				...
				((Graphic)sliderBg).color = Color.Lerp(emptyColor, fullColor, heatUp);
				if (heatUp <= 0f && heatSinks < 1f)
				{
					((Graphic)sliderBg).color = new Color(0f, 0f, 0f, 0f);
				}
			}
			else if (heatUp == 0f && heatSinks >= 1f && ((Graphic)sliderBg).color.a == 0f)
			{
				((Graphic)sliderBg).color = emptyColor;
			}
		}
		...
		barrelAud.SetPitch(spinSpeed / 1500f * 2f);
		Spin[] array = barrels;
	...
	private void UpdateZapHud()
	{
		if (wc.naiZapperRecharge < 5f)
		{
			...
			statusText.text = "";
			((Component)(object)zapMeter).gameObject.SetActive(value: false);
			((Behaviour)(object)warningX).enabled = false;
		...
		if (currentZapper == null)
		{
			...
			((Behaviour)(object)warningX).enabled = false;
			float num = 0f;
			if (Physics.Raycast(cc.GetDefaultPos(), cc.transform.forward, out var hitInfo, float.PositiveInfinity, LayerMaskDefaults.Get(LMD.EnemiesAndEnvironment)) && !LayerMaskDefaults.IsMatchingLayer(hitInfo.collider.gameObject.layer, LMD.Environment) && hitInfo.collider.TryGetComponent<EnemyIdentifierIdentifier>(out var component) && (bool)component.eid && !component.eid.dead)
			{
				...
				else
				{
					...
					num = 0f;
					((Graphic)statusText).color = MonoSingleton<ColorBlindSettings>.Instance.variationColors[2];
				}
			}
			else
			{
				...
				num = 0f;
				((Graphic)statusText).color = Color.gray;
			}
		...
		else
		{
			...
			{
				((Component)(object)zapMeter).gameObject.SetActive(value: true);
			...
			{
				((Behaviour)(object)warningX).enabled = true;
				((Graphic)warningX).color = ((currentZapper.breakTimer % 0.1f > 0.05f) ? Color.red : Color.white);
			...
			else
			{
				...
				((Graphic)statusText).color = Color.Lerp(Color.red, Color.white, (currentZapper.maxDistance - currentZapper.distance) / currentZapper.maxDistance);
			}
		}
	}
	...
	private void Shoot()
	{
		...
		MonoSingleton<PlayerAnimations>.Instance?.Shoot(2f);
		fireCooldown = currentFireRate;
		...
		else if (portalTraversals2.Length != 0)
		{
			...
			else if (blocked)
			{
				...
				quaternion2 = Quaternion.LookRotation(Vector3.Reflect(forward, float3.op_Implicit(nativePortalTransform.back)).normalized);
			}
		}
	...
	public void BurstFire()
	{
		...
		gameObject.GetComponent<Rigidbody>().AddForce(vector3 * -100f, ForceMode.VelocityChange);
	...
	public void SuperSaw()
	{
		...
		{
			component.velocity = vector3 * 200f;
		}
	...
	public void ShootMagnet()
	{
		...
		else if (portalTraversals.Length != 0)
		{
		...
		gameObject.GetComponent<Rigidbody>().AddForce(vector2 * 100f, ForceMode.VelocityChange);
		if (canShoot)
		{
			anim.SetTrigger("Shoot");
		}
		Object.Instantiate<AudioSource>(magnetShotSound);
	...
	private void RefreshHeatSinkFill(float charge, bool playSound = false)
	{
		...
		for (int i = 0; i < heatSinkImages.Length; i++)
		{
			if (heatSinkFill > (float)i)
			{
				...
				if (heatSinkFill >= (float)(i + 1) && ((Graphic)heatSinkImages[i]).color != MonoSingleton<ColorBlindSettings>.Instance.variationColors[num])
				{
					if (playSound)
					{
						aud.SetPitch((float)i * 0.5f + 1f);
						aud.Play(tracked: true);
					}
					((Graphic)heatSinkImages[i]).color = MonoSingleton<ColorBlindSettings>.Instance.variationColors[num];
				}
				else if (heatSinkFill < (float)(i + 1))
				{
					((Graphic)heatSinkImages[i]).color = emptyColor;
				}
			}
			else
			{
		...
		if ((bool)(Object)(object)ammoText)
		{
			((Graphic)ammoText).color = MonoSingleton<ColorBlindSettings>.Instance.variationColors[CorrectVariation()];
		}
	}

	public void SnapSound()
	{
		Object.Instantiate<AudioSource>(snapSound);
	}
