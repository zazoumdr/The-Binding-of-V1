...
public class RandomSoundPlayer : MonoBehaviour
{
	...
	private void Update()
	{
		...
		aud.volume = fade;
		if (playing && aud.GetPitch() > targetPitch + 0.1f)
		{
			aud.SetPitch(aud.GetPitch() - Time.deltaTime / 10f);
		}
		else if (playing && aud.GetPitch() < targetPitch - 0.1f)
		{
			aud.SetPitch(aud.GetPitch() + Time.deltaTime / 10f);
		}
	}
	...
	private void PlayRandomSound()
	{
		if (!playing)
		{
			...
			aud.SetPitch(Random.Range(0.2f, 3f));
			targetPitch = aud.GetPitch();
