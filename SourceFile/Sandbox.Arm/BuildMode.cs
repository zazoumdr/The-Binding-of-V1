...
public class BuildMode : ArmModeWithHeldPreview
{
	...
	public override void Update()
	{
		...
		Vector3 endPoint;
		bool active = PortalPhysicsV2.Raycast(transform.position, transform.forward, 75f, hostArm.raycastLayers, out hitInfo, out portalTraversals, out endPoint);
		...
		if (secondBlockPos != previousSecondBlockPos)
		{
			if (tickDelay == 0f)
			{
				hostArm.tickSound.SetPitch(Random.Range(0.74f, 0.76f));
				hostArm.tickSound.Play(tracked: true);
				tickDelay = 0.05f;
			}
	...
	public override void OnPrimaryDown()
	{
		...
		{
			SpawnableInstance component2 = gameObject.GetComponent<SpawnableInstance>();
