...
public class PlaceMode : ArmModeWithHeldPreview
{
	...
	public override void OnPrimaryDown()
	{
		...
		Vector3 normal = ((currentObject.spawnableObjectType == SpawnableObject.SpawnableObjectDataType.Enemy) ? Vector3.up : hostArm.hit.normal);
		GameObject gameObject = Object.Instantiate(currentObject.gameObject, CalculatePropPosition(hostArm.hit), CalculatePropRotation(normal, currentObject.gameObject.transform.localRotation));
		...
		if (flag)
		{
			(gameObject.GetComponent<EnemySpawnableInstance>() ?? gameObject.AddComponent<EnemySpawnableInstance>()).sourceObject = currentObject;
		...
		if (gameObject.TryGetComponent<SpawnableInstance>(out var component2))
		{
