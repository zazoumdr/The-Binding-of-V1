...
{
	private static readonly Logger Log = new Logger("SandboxUtils");
	...
	public static void StripForPreview(Transform target, Material newMaterial = null, bool first = true)
	{
		...
		if (first)
		{
			...
			{
				UnityEngine.Object.Destroy(((Component)(object)componentsInChildren[i]).gameObject);
			}
			...
			list.AddRange((IEnumerable<Component>)(object)target.GetComponentsInChildren<AudioDistortionFilter>(includeInactive: true));
			list.AddRange((IEnumerable<Component>)(object)target.GetComponentsInChildren<AudioHighPassFilter>(includeInactive: true));
			list.AddRange((IEnumerable<Component>)(object)target.GetComponentsInChildren<AudioLowPassFilter>(includeInactive: true));
			list.AddRange((IEnumerable<Component>)(object)target.GetComponentsInChildren<ProBuilderMesh>(includeInactive: true));
	...
	public static GameObject CreateFinalBlock(SpawnableObject proceduralTemplate, Vector3 position, Vector3 size, bool liquid = false)
	{
		...
		{
			Log.Info($"Creating block {size}", (IEnumerable<Tag>)null, (string)null, (object)null);
		}
