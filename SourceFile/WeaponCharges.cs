...
public class WeaponCharges : MonoSingleton<WeaponCharges>
{
	...
	public TimeSince sinceAltFirePressed;
	...
	public void Charge(float amount)
	{
		...
		if (shoAltYellowsTimer > 0f)
		{
			shoAltYellowsTimer = Mathf.MoveTowards(shoAltYellowsTimer, 0f, amount);
			if (shoAltYellowsTimer <= 0f)
			{
				shoAltYellows = 0;
			}
		...
		if (shoSawResetTimer > 0f)
		{
			shoSawResetTimer = Mathf.MoveTowards(shoSawResetTimer, 0f, amount);
		}
		if (naiHeatsinks < 2f)
		{
		...
		if (rocketFrozen && (bool)rocketLauncher && (bool)(Object)(object)rocketLauncher.currentTimerTickSound)
		{
			rocketLauncher.currentTimerTickSound.SetPitch(Mathf.Lerp(0.75f, 1f, rocketFreezeTime / 5f));
		}
		...
		if (punchStamina < 2f)
		{
			punchStamina = Mathf.MoveTowards(punchStamina, 2f, amount * 1.25f);
		}
	}
