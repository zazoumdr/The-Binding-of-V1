...
public class Wicked : MonoBehaviour
{
	...
	private void Update()
	{
		if (alwaysRunning || playerSpotTime > 0f)
		{
		...
		if (!BlindEnemies.Blind && !Physics.Raycast(base.transform.position + Vector3.up * 2f, player.transform.position - base.transform.position + Vector3.up * 2f, Vector3.Distance(base.transform.position + Vector3.up * 2f, player.transform.position), LayerMaskDefaults.Get(LMD.Environment)))
		{
